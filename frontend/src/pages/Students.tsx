        ...
        if (chartElement && evolutionData.length > 0) {
            try {
                ...
                const canvas = await html2canvas(chartElement, {
                    backgroundColor: getComputedStyle(document.documentElement).getPropertyValue('--color-bg-card') || '#1f2937' // Match bg-bg-card
                });
                            ...
                            {filteredEvolutionData.length > 0 ? (
                                <ResponsiveContainer width="100%" height={360}>
                                    <LineChart data={filteredEvolutionData}>
                                        ...
                                        <XAxis dataKey="date" stroke="var(--color-text-muted)" fontSize={11} tick={{ fill: 'var(--color-text-muted)' }} axisLine={{ stroke: 'var(--color-border)' }} />
                                        <YAxis stroke="var(--color-text-muted)" domain={[0, 10]} fontSize={11} tick={{ fill: 'var(--color-text-muted)' }} axisLine={{ stroke: 'var(--color-border)' }} />
                                        <Tooltip
                                            contentStyle={{
