    ...
    return (
        <div className="glass-card p-6 mt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {plans.map(plan => (
                    <div key={plan.id} className="border border-white/10 p-4 rounded-lg bg-bg-dark/50">
                        <div className="flex justify-between items-center mb-2">
                            <h3 className="font-bold text-lg text-primary">{plan.name}</h3>
                            <button onClick={() => setEditingPlan(plan)} className="text-text-muted hover:text-white">
                                <Edit size={16} />
                            </button>
                        </div>
                        ...
                        <ul className="mt-4 text-sm text-text-muted space-y-1">
                            {plan.features?.map((f, i) => (
                                <li key={i} className={f.included ? 'text-white' : 'text-text-muted/50'}>
                                    {f.included ? '✓' : '×'} {f.text}
                                </li>
                            ))}
                        </ul>
            ...
            {editingPlan && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
                    <div className="glass-card w-full max-w-lg p-6 relative">
                        <button onClick={() => setEditingPlan(null)} className="absolute top-4 right-4 text-text-muted hover:text-white">
                            <X size={20} />
                        </button>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input 
                                    className="w-full p-2 bg-bg-dark border border-white/10 rounded" 
                            ...
                            <button 
                                onClick={handleSave}
                                disabled={loading}
                                className="w-full bg-primary hover:bg-primary-hover text-white py-2 rounded mt-4"
                            >
